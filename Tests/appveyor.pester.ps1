...
if(-not $Finalize) {
    ...
    Import-Module (Join-Path -Path $ProjectRoot -ChildPath 'CoverageChecks\ECI-Utilities') -Force
    ...
    $CodeResults = Invoke-ScriptAnalyzer -Path "$ProjectRoot\CoverageChecks" -Recurse -Severity Error -ErrorAction SilentlyContinue
    If ($CodeResults) {
    ...
    $TestResults = Invoke-Pester -Path ".\Tests" -CodeCoverage ".\*\ECI-Utilities\*\*.ps1" -PassThru -Show None
    ...
    if ($null -ne $TestResults) {
        ...
        Write-Host "No tests = no problems!" -ForegroundColor Green
    }
}